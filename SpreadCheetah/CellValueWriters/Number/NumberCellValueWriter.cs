...
namespace SpreadCheetah.CellValueWriters.Number
{
    internal abstract class NumberCellValueWriter : CellValueWriter
    {
        ...
        {
            buffer.Index += SpanHelper.GetBytes(DataCellHelper.BeginNumberCell, buffer.GetNextSpan());
            return true;
        }
        ...
        public static bool DoWriteFormulaStartElement(StyleId? styleId, SpreadsheetBuffer buffer)
        {
            if (styleId is null)
            {
                buffer.Index += SpanHelper.GetBytes(FormulaCellHelper.BeginNumberFormulaCell, buffer.GetNextSpan());
                return true;
            }
